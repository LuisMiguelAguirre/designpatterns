    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Printing the details
                  ...
                  Console.WriteLine(Principal.getDetails());
                  List<ITeacher> hods=Principal.getControllingDepts();
                  for(int i=0;i<hods.Count;i++)
                  {
                      Console.WriteLine("\t" + hods.ElementAt(i).getDetails());
                  }

            List<ITeacher> mathTeachers=hodMaths.getControllingDepts();
                  for(int i=0;i<mathTeachers.Count;i++)
                  {
                      Console.WriteLine("\t\t" + mathTeachers.ElementAt(i).getDetails());
                  }

             List<ITeacher> cseTeachers=hodCompSc.getControllingDepts();
                  for(int i=0;i<cseTeachers.Count;i++)
                  {
                      Console.WriteLine("\t\t" + cseTeachers.ElementAt(i).getDetails());
                  }
